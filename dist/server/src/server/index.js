...
const CanvasState_1 = require("./CanvasState");
...
const canvasState = new CanvasState_1.CanvasState();
const userManager = new userManager_1.UserManager();
const activeStrokes = new Map();
function validateDrawStartEvent(data) {
    return data &&
...
function validateDrawMoveEvent(data) {
    return data &&
        Array.isArray(data.points) &&
        data.points.length > 0 &&
        data.points.length <= 1000 &&
        data.points.every((p) => p &&
            typeof p.x === 'number' &&
            typeof p.y === 'number' &&
            !isNaN(p.x) && !isNaN(p.y) &&
            isFinite(p.x) && isFinite(p.y));
}
function validateCursorMoveEvent(data) {
    return data &&
        typeof data.x === 'number' &&
        typeof data.y === 'number' &&
        !isNaN(data.x) && !isNaN(data.y) &&
...
app.use((req, res) => {
    res.status(404).sendFile(path_1.default.join(__dirname, '../../../../public/index.html'));
});
